...
- 모든 쓰기 연산을 순차적으로 파일에 기록하는 방식
- 재시작 시 AOF에 기록된 모든 연산을 재수행하여 데이터를 복구
- 디폴트로는 appendonly no 로 비활성화
- 데이터 무결성이나 정합성이 중요한 경우에 적절

- 장점
  - 모든 변경 사항을 기록하기 때문에 안정적으로 데이터를 백업할 수 있음
- 단점
  - 모든 히스토리가 저장되기 때문에 RDB 백업 파일 보다 파일 사이즈가 큼
  - RDB 방식 대시 백업과 복구 속도가 느림
  - 모든 write 를 디스크에 기록해야해서 쓰기 성능이 떨어짐
...
- 특정 시점의 데이터를 덤프 파일로 저장하는 방식
- 재시작 시 덤프 파일을 통해 데이터를 복구
- 디폴트로 RDB 방식이 활성화되어있음

- 장점
  - 스냅샷한 데이터 외의 부가적인 데이터가 없기에 작은 파일 사이즈로 백업 파일 관리 가능
  - 빠른 복구 가능
- 단점
  - 스냅샷을 저장하는 시점 사이의 데이터 변경 사항 유실이 있을 수 있음
  - 대규모 데이터 스냅샷일 경우 디스크 I/O 증가
